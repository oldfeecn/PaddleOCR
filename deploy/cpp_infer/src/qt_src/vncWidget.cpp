...
void vncWidget::initPaddleDll()
{
    // 加载 DLL
    if (paddleWrapper.loadLibrary("C:\\CUDA\\PaddleDetection\\deploy\\cpp\\build\\Release\\main_lib.dll")) {
        // 初始化参数
        std::vector<std::string> arg_strings = {
        "./main",
        "--model_dir=D:/workspace/PaddleDetection/output_inference/rtdetr_r50vd_6x_coco",
        "--image_file=D:\\img\\20240803_161733_963.bmp",
        "--cpu_threads=4",
        "--device=GPU"
         };
        qDebug() << "initdll successful";
        if (paddleWrapper.initializeArgs(arg_strings)) {
        ...
        // 准备图像数据
        cv::Mat image = cv::imread("D:\\img\\20240803_161733_963.bmp");

        // 处理图像
        std::vector<PaddleDetection::ObjectResult> results = paddleWrapper.processArguments(image);

        // 使用结果
        for (const auto& result : results) {
            qDebug() << "Class ID:" << result.class_id;
            qDebug() << "Label:" << QString::fromStdString(result.label);
            qDebug() << "Confidence:" << result.confidence;
            qDebug() << "Rect:" << QString::fromStdString(
                std::to_string(result.rect[0]) + ", " +
                std::to_string(result.rect[1]) + ", " +
                std::to_string(result.rect[2]) + ", " +
                std::to_string(result.rect[3])
            );
        }
    } else {
...
cv::Mat vncWidget::QPixmapToCvMat(const QPixmap& pixmap)
{
    // 将 QPixmap 转换为 QImage
    QImage image = pixmap.toImage();

    // 确保 QImage 使用的是 8-bit RGB 格式
    ...
    // 创建 OpenCV 的 cv::Mat 对象
    cv::Mat mat(imageHeight, imageWidth, CV_8UC3, (void*)imageData, imageBytesPerLine);

    // 注意：OpenCV 默认是 BGR 格式，需要将 RGB 转换为 BGR
    cv::Mat bgrMat;
...
{
    {
        ...
        if (!pixmap.isNull())
        {
            ...
            //    };
            //}
            paddleWrapper.processArguments(QPixmapToCvMat(pixmap));
            update();
        }
