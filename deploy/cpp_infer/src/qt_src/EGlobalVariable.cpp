...
void EGlobalVariable::getScreenWH()
{
    ...
    //screen_height = mm.height();//获取的高度去掉了下方的任务栏高度(这里不适用)
    screen_height = QApplication::desktop()->height();;

    qreal dpiVal = screen->logicalDotsPerInch();//获取系统字体放大倍数
        //获取系统dpi放大倍速//屏幕的放大系数100% 125% 150% 200%，对应的dpi值为：96 120 144 192.
    if (dpiVal == 96) {
