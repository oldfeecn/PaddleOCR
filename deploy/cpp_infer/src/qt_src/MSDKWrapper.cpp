...
bool MSDKWrapper::loadLibrary(const QString &path)
{
    ...
    if (!Q_M_Open) { qDebug() << "Failed to resolve M_Open";  hasError =true; }
    if (!Q_M_Open_VidPid) { qDebug() << "Failed to resolve M_Open_VidPid";  hasError =true; }
    if (!Q_M_ScanAndOpen) { qDebug() << "Failed to resolve M_ScanAndOpen";  hasError =true; }
    if (!Q_M_Close) { qDebug() << "Failed to resolve M_Close";  hasError =true; }
    if (!Q_M_GetDevSn) { qDebug() << "Failed to resolve M_GetDevSn";  hasError =true; }
    if (!Q_M_SetUserData) { qDebug() << "Failed to resolve M_SetUserData";  hasError =true; }
    if (!Q_M_VerifyUserData) { qDebug() << "Failed to resolve M_VerifyUserData";  hasError =true; }
    if (!Q_M_ChkSupportMdy) { qDebug() << "Failed to resolve M_ChkSupportMdy";  hasError =true; }
    if (!Q_M_GetAbsCfg) { qDebug() << "Failed to resolve M_GetAbsCfg";  hasError =true; }
    if (!Q_M_SetAbsCfg) { qDebug() << "Failed to resolve M_SetAbsCfg";  hasError =true; }
    if (!Q_M_SetNewVidPid) { qDebug() << "Failed to resolve M_SetNewVidPid";  hasError =true; }
    if (!Q_M_ResetVidPid) { qDebug() << "Failed to resolve M_ResetVidPid";  hasError =true; }
    if (!Q_M_GetVidPid) { qDebug() << "Failed to resolve M_GetVidPid";  hasError =true; }
    if (!Q_M_ChkSupportProdStrMdy) { qDebug() << "Failed to resolve M_ChkSupportProdStrMdy";  hasError =true; }
    if (!Q_M_GetProdStr) { qDebug() << "Failed to resolve M_GetProdStr";   }
    if (!Q_M_SetProdStr) { qDebug() << "Failed to resolve M_SetProdStr";  hasError =true; }
    if (!Q_M_ChkSupportEnc) { qDebug() << "Failed to resolve M_ChkSupportEnc";  hasError =true; }
    if (!Q_M_ChkPwStat) { qDebug() << "Failed to resolve M_ChkPwStat";  hasError =true; }
    if (!Q_M_SetPw) { qDebug() << "Failed to resolve M_SetPw";  hasError =true; }
    if (!Q_M_VerifyPw) { qDebug() << "Failed to resolve M_VerifyPw";  hasError =true; }
    if (!Q_M_WrEncData) { qDebug() << "Failed to resolve M_WrEncData";  hasError =true; }
    if (!Q_M_RdEncData) { qDebug() << "Failed to resolve M_RdEncData";  hasError =true; }
    if (!Q_M_InitParam) { qDebug() << "Failed to resolve M_InitParam";  hasError =true; }
    if (!Q_M_SetParam) { qDebug() << "Failed to resolve M_SetParam";  hasError =true; }
    if (!Q_M_KeyPress) { qDebug() << "Failed to resolve M_KeyPress";  hasError =true; }
    if (!Q_M_KeyDown) { qDebug() << "Failed to resolve M_KeyDown";  hasError =true; }
    if (!Q_M_KeyUp) { qDebug() << "Failed to resolve M_KeyUp";  hasError =true; }
    if (!Q_M_KeyState) { qDebug() << "Failed to resolve M_KeyState";  hasError =true; }
    if (!Q_M_KeyPress2) { qDebug() << "Failed to resolve M_KeyPress2";  hasError =true; }
    if (!Q_M_KeyDown2) { qDebug() << "Failed to resolve M_KeyDown2";  hasError =true; }
    if (!Q_M_KeyUp2) { qDebug() << "Failed to resolve M_KeyUp2";  hasError =true; }
    if (!Q_M_KeyState2) { qDebug() << "Failed to resolve M_KeyState2";  hasError =true; }
    if (!Q_M_ReleaseAllKey) { qDebug() << "Failed to resolve M_ReleaseAllKey";  hasError =true; }
    if (!Q_M_NumLockLedState) { qDebug() << "Failed to resolve M_NumLockLedState";  hasError =true; }
    if (!Q_M_CapsLockLedState) { qDebug() << "Failed to resolve M_CapsLockLedState";  hasError =true; }
    if (!Q_M_ScrollLockLedState) { qDebug() << "Failed to resolve M_ScrollLockLedState";  hasError =true; }
    if (!Q_M_KeyInputString) { qDebug() << "Failed to resolve M_KeyInputString";  hasError =true; }
    if (!Q_M_KeyInputStringGBK) { qDebug() << "Failed to resolve M_KeyInputStringGBK";  hasError =true; }
    if (!Q_M_KeyInputStringUnicode) { qDebug() << "Failed to resolve M_KeyInputStringUnicode";  hasError =true; }
    if (!Q_M_LeftClick) { qDebug() << "Failed to resolve M_LeftClick";  hasError =true; }
    if (!Q_M_LeftDoubleClick) { qDebug() << "Failed to resolve M_LeftDoubleClick";  hasError =true; }
    if (!Q_M_LeftDown) { qDebug() << "Failed to resolve M_LeftDown";  hasError =true; }
    if (!Q_M_LeftUp) { qDebug() << "Failed to resolve M_LeftUp";  hasError =true; }
    if (!Q_M_RightClick) { qDebug() << "Failed to resolve M_RightClick";  hasError =true; }
    if (!Q_M_RightDown) { qDebug() << "Failed to resolve M_RightDown";  hasError =true; }
    if (!Q_M_RightUp) { qDebug() << "Failed to resolve M_RightUp";  hasError =true; }
    if (!Q_M_MiddleClick) { qDebug() << "Failed to resolve M_MiddleClick";  hasError =true; }
    if (!Q_M_MiddleDown) { qDebug() << "Failed to resolve M_MiddleDown";  hasError =true; }
    if (!Q_M_MiddleUp) { qDebug() << "Failed to resolve M_MiddleUp";  hasError =true; }
    if (!Q_M_ReleaseAllMouse) { qDebug() << "Failed to resolve M_ReleaseAllMouse";  hasError =true; }
    if (!Q_M_MouseKeyState) { qDebug() << "Failed to resolve M_MouseKeyState";  hasError =true; }
    if (!Q_M_MouseWheel) { qDebug() << "Failed to resolve M_MouseWheel";  hasError =true; }
    if (!Q_M_ResetMousePos) { qDebug() << "Failed to resolve M_ResetMousePos";  hasError =true; }
    if (!Q_M_MoveR) { qDebug() << "Failed to resolve M_MoveR";  hasError =true; }
    if (!Q_M_MoveTo) { qDebug() << "Failed to resolve M_MoveTo";  hasError =true; }
    if (!Q_M_GetCurrMousePos) { qDebug() << "Failed to resolve M_GetCurrMousePos";  hasError =true; }
    if (!Q_M_GetCurrMousePosX) { qDebug() << "Failed to resolve M_GetCurrMousePosX";  hasError =true; }
    if (!Q_M_GetCurrMousePosY) { qDebug() << "Failed to resolve M_GetCurrMousePosY";  hasError =true; }
    if (!Q_M_MoveR2) { qDebug() << "Failed to resolve M_MoveR2";  hasError =true; }
    if (!Q_M_MoveTo2) { qDebug() << "Failed to resolve M_MoveTo2";  hasError =true; }
    if (!Q_M_GetCurrMousePos2) { qDebug() << "Failed to resolve M_GetCurrMousePos2";  hasError =true; }
    if (!Q_M_ResolutionUsed) { qDebug() << "Failed to resolve M_ResolutionUsed";  hasError =true; }
    if (!Q_M_MoveTo3) { qDebug() << "Failed to resolve M_MoveTo3";  hasError =true; }
    if (!Q_M_MoveTo3_D) { qDebug() << "Failed to resolve M_MoveTo3_D";  hasError =true; }
    if (!Q_M_Delay) { qDebug() << "Failed to resolve M_Delay";  hasError =true; }
    if (!Q_M_DelayRandom) { qDebug() << "Failed to resolve M_DelayRandom";  hasError =true; }
    if (!Q_M_RandDomNbr) { qDebug() << "Failed to resolve M_RandDomNbr";  hasError =true; }
